/// BIP39 English wordlist (2048 words) - official standard.
/// Source: github.com/trezor/python-mnemonic (MIT license / public domain)
///
/// Encoding: 32 bytes (256 bits) + 8-bit SHA-256 checksum = 264 bits → 24 words × 11 bits.
...
/// Encode 32 bytes as 24 BIP39 words.
pub fn entropy_to_mnemonic(entropy: &[u8; 32]) -> Vec<String> {
    let n = WORDS.len();
    assert!(n == 2048, "wordlist must be exactly 2048 words");

    let mut hasher = CryptographicHash::new("SHA256", 1)
        .expect("SHA256");
    ...
    for b in 0..8 {
        bits[256 + b] = (checksum_byte >> (7 - b)) & 1 == 1;
    }

    (0..24).map(|i| {
        let start = i * 11;
        let mut idx = 0usize;
        for b in 0..11 { idx = (idx << 1) | (bits[start + b] as usize); }
        WORDS[idx].to_string()
    }).collect()
}

/// Decode 24 BIP39 words to 32 bytes of entropy, verifying the checksum.
pub fn mnemonic_to_entropy(words: &[String]) -> Result<[u8; 32], String> {
    if words.len() != 24 {
        return Err(format!("24 words required, got {}", words.len()));
    }

    let mut bits = [false; 264];
    for (i, word) in words.iter().enumerate() {
        let w = word.to_lowercase();
        let idx = WORDS.iter().position(|&x| x == w.as_str())
    ...
    let mut actual = 0u8;
    for b in 0..8 {
        if bits[256 + b] { actual |= 1 << (7 - b); }
    }
    if expected != actual {
        return Err("Invalid checksum - incorrect phrase or misspelled word".to_string());
    }

    Ok(entropy)
}

/// Returns the wordlist size (always 2048 for standard BIP39).
pub fn wordlist_len() -> usize { WORDS.len() }
